...
namespace QuizVerse.UnitTests.Services
{
    public class QuizCategoryServiceTest
    {
        ...
        public async Task GetQuizCategories_WithInvalidSortColumn_ShouldThrowException()
        {
            ...
            Assert.Equal("SortColumn", ex.ParamName);
            Assert.Contains("InvalidColumn", ex.Message);
        }

        [Fact]
        public async Task GetQuizCategories_WhenNoDataFound_ShouldThrowAppException()
        {
            // Arrange
            var emptyQuizCategories = new List<QuizCategory>().AsQueryable();
            var request = new PageListRequest { PageNumber = 1, PageSize = 10 };

            _quizCategoryRepoMock.Setup(r => r.GetQueryableInclude(It.IsAny<Expression<Func<QuizCategory, object>>>()))
                .Returns(emptyQuizCategories);

            // Act & Assert
            var ex = await Assert.ThrowsAsync<AppException>(() => _service.GetQuizCategories(request));
